  ...
  module CLI
    module Commands
      ...
      class Indicators < Base
        # Ruby built-in and helper methods to exclude when listing SQAI's TA-Lib indicators
        EXCLUDED_METHODS = %i[
        ...
        private_constant :EXCLUDED_METHODS

        # Category name => matcher. A matcher is either a Set of indicator names,
        # or a callable that receives the indicator name (String) and returns truthy/falsy.
        CATEGORY_MATCHERS = {
          'Overlap Studies (Moving Averages & Bands)' =>
            %w[sma ema wma dema tema trima kama mama t3 ma mavp bbands],
          'Momentum Indicators' =>
            %w[adx adxr apo aroon aroonosc bop cci cmo dx macd macdext macdfix mfi minus_di minus_dm mom
               plus_di plus_dm ppo roc rocp rocr rocr100 rsi stoch stochf stochrsi trix ultosc willr],
          'Volume Indicators' => %w[ad adosc obv],
          'Volatility Indicators' => %w[atr natr trange],
          'Price Transform' => %w[avgprice medprice typprice wclprice],
          'Cycle Indicators' => ->(name) { name.start_with?('ht_') },
          'Pattern Recognition (Candlestick)' => ->(name) { name.start_with?('cdl_') },
          'Statistic Functions' =>
            %w[beta correl linearreg linearreg_angle linearreg_intercept linearreg_slope stddev tsf var],
          'Math Transform' =>
            %w[accbands acos asin atan ceil cos cosh exp floor ln log10 sin sinh sqrt tan tanh sar sarext],
          'Other' => %w[imi midpoint midprice]
        }.freeze
        private_constant :CATEGORY_MATCHERS
